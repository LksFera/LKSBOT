	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw meu nobre @${num.split('@')[0]} Pá fora 👉🚪`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '⚠️ Ala menó poder nenhum KKKKKK. Este comando só pode ser usado por Admins',
			...
			switch(command) {
				...
				case 'meme':
					...
					break
               case 'melhor adm':
		            if (args.length < 1) return reply ('Tropa Marilu 🙅')
	               	break
				case 'porno':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQB6PMHXR8A2CzZ0F23EhEvSb3CLbKF4vnUUg&usqp=CAU`)
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR DA BASE:* Dark\n*YOUTUBE:* https://bit.ly/2MD43V2\n*WPP:* wa.me/+5511954580970\n*INSTA:* Lucasmolino18\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
